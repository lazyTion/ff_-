<template>
  <view :class="languageClass">
    <view class="test">
      ...
      <button type="primary" class="button-text" @click="onShowDatePicker('rangetime')">{{ lang.selectDateTimeRange }}</button>
    </view>
    <mx-date-picker
        :show="showPicker"
        :type="type"
    ...
    <view class="table">
      <view v-for="val in valdata" :key="val.index">
        ...
        <view class="tr">
          <view class="td content-text">{{ lang.carNumber }}：{{ val.ch }}</view>
          <view class="td content-text">{{ lang.specification }}：{{ val.guige }}</view>
          <view class="td content-text">{{ lang.quantity }}：{{ val.ddwlsl }}</view>
      </view>
  </view>
</template>
<script>
...
export default {
  ...
  data() {
    return {
	  ...
	  currentLang: 'zh', // 当前语言
	  lang: zh // 语言资源对象
				languageClass: 'lang-zh lang-transition page-chaxun'
    }
  onLoad: function (options) {
    ...
    that.rangetime[1] = time2;
		this.currentLang = uni.getStorageSync('lang') || 'zh';
			...
			this.languageClass = getLanguageClass(this.currentLang, 'page-chaxun');
